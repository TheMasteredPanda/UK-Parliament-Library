...
async def er_task(er_member: PartyMember, session: aiohttp.ClientSession):
    async with session.get(f"{utils.URL_MEMBERS}/Location/Constituency/{er_member._get_membership_from_id()}/ElectionResults") as elections_resp:
        if elections_resp.status != 200:
            raise Exception(f"Couldn't fetch election results for MP Borough {er_member.get_addressed_name()}/{er_member.get_id()}. Status Code: {elections_resp.status}")

        elections_obj = await elections_resp.json() 

        async def inner_task(session: aiohttp.ClientSession, borough_id: int, election_id: int):
            async with session.get(f"{utils.URL_MEMBERS}/Location/Constituency/{borough_id}/ElectionResult/{election_id}") as election_resp:
                if election_resp.status != 200:
                    raise Exception(f"Couldn't fetch election result {election_id}. Status Code: {election_resp.status}")
        ...
        for election_json_obj in elections_obj['value']:
            election_tasks.append(inner_task(session, er_member._get_membership_from_id(), election_json_obj['electionId']))
